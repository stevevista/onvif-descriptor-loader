...
class Element {
    ...
    if (this.nsName === nsName) {
      ...
      if (this !== stack[0]) {
        if (this.name === 'documentation') {
          parent.postProcessDocumentation(this);
        } else if (this.name === 'annotation') {
          parent.htmldoc = this.htmldoc
        } else {
  ...
  postProcess(definitions) {
    console.debug('Unknown element: %s %s', this.nsURI, this.nsName)
  }

  postProcessDocumentation (doc) {
    for (const el of doc.children) {
      if (typeof el.renderHtml === 'function') {
        if (this.htmldoc === undefined) {
          this.htmldoc = ''
        }
        this.htmldoc += el.renderHtml();
      }
    }
  }
}
