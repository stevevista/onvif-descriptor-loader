...
class Attribute extends XSDElement {
  ...
  describe(definitions) {
    ...
    if (this.ref) {
      ...
      // @RJ Fix bug on retrive attribute types
      if (typeQName) {
        const simpleType = xsd.getBuiltinType(typeQName.name)
        if (simpleType) {
          this.descriptor.jsType = simpleType.jsType
        }
      }
    }

    this.descriptor.htmldoc = this.htmldoc
